...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h' or opt == '--help':
            ...
            logger.debug("\t flight_date_leg_id %d" % flight_date_leg_id)
            ...
            seat_map_id = int(arg)
            logger.debug("\t seat_map_id %d" % seat_map_id)
        elif opt in ("-N", "--days"):
            recCount = int(arg)
            logger.info("\t count %d" % recCount)
    ...
    if cfg_table and aircraft_code is not None:
        logger.debug("Get config table for aircraft code %s" % aircraft_code)
        ...
        if list_flights and dt1 is not None:
            if dt2 is not None:
                logger.debug("List flights from %s to %s"
                ...
                logger.debug("List flights for %s" % dt1.strftime("%Y-%m-%d"))
    ...
    # Flight number specified
        ...
        if list_pax and dt1 is not None:
            ...
            logger.debug("Read flight %s from %s to %s"
                     % (flight_number, dt1.strftime("%Y-%m-%d"),
                        dt2.strftime("%Y-%m-%d")))
            n = 0
            for single_date in daterange(dt1, dt2):
            ...
            city_pair = flights[0].city_pair
        if city_pair == 0:
            logger.info("City pair not found", 1)
