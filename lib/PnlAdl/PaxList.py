...
class PaxList(object):
    ...
    def GetClassCount(self):
        ...
        for it in self.paxListEntries:
            ...
            if mapit is None:
                ...
                self.booking_classes[booking_class] += 1
            logger.debug("Booking class %c : %d"
    ...
    def Show(self):
        ...
        while i < len(self.paxListEntries):
            ...
            if it.book_no == previt.book_no:
                ...
                logger.debug("Set group name for %s to %s"
                         % (previt.pax_name, previt.group_name))
                # it.locator = str("")
                it.group_name = previt.group_name
                logger.debug("Set group name for %s to %s and locator to '%s'"
    ...
    def ReadDb(self, aFlightNumber, aBoardDate, aDepartAirport):
        ...
        self.AltFlightNumber = ReadAltFlightNumber(self.conn,
                                                   self.FlightNumber,
                                                   self.BoardDate)
        logger.info("Get pax data for flight '%s' date '%s'"
    ...
    def ReadBuf(self, inBuf, doPnl):
        ...
        for it in segments:
            n += 1
            logger.debug("\t\t%d: %s" % n, it.c_str())

        n = len(segments)
        if n < 13:
            print("Too few lines in file (%d)" % n)
            return 1
        logger.debug("\tread %d lines [%s]" % n, segments[n-2])
        ...
        if segments[n-2][0:4] == "NNNN":
            n -= 1
            logger.debug("\tRead %d lines : %s" % n, segments[n-1])
        else:
            logger.info("Read %d lines [%s]" % n, segments[n-2])
        ...
        while i < n - 5:
            ...
            if fdata[0] >= '0' and fdata[0] <= '9':
                ...
                classtot[4] = 0
                nclass = int(classtot)
                logger.info("Class %c %d [%s]" % classnam, nclass, fdata)
    ...
    def AddGroup(self, pd):
        ...
        if len(pd.locator) == 0:
            ...
            if it is None:
                logger.info("\tno locator for group '%s'" % pd.group_name)
                return
            else:
                pd.locator = it.second
                logger.info("\tlocator for group '%s' specified above as '%s'"
        ...
        if it is not None:
            ...
            logger.info("\tlocator for group '%s' set to '%s'"
