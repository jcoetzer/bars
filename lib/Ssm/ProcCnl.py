...
def DeleteSellingClasses(conn,
    ...
    logger.debug("Deleted %d row(s)" % rowcount)
    cur.close()
    return rowcount
...
def ProcCnl(conn,
    ...
    logger.debug("Cancel flight %s" % ssm.flight_number)
    ...
    if flight_period_id == 0:
        logger.error("Flight period for %s does not exist" % ssm.flight_number)
        #return -1

    if sdate != ssm.start_date:
        logger.error("Start date from input %s does not match stored value %s"
            % (ssm.start_date, sdate))
    elif edate != ssm.end_date:
        logger.error("End date from input %s does not match stored value %s"
            % (ssm.end_date, edate))
    else:
        pass
    ...
    #for flight_date in DateRange(ssm.start_date, ssm.end_date):
        #fday = str(flight_date.strftime('%w'))
            ...
            #fday = '7'
        #if fday in freq:
            #logger.debug("Delete flight date %s (day %s)" % (flight_date, fday))
    ...
    while cdate <= ssm.end_date:
        ...
        logger.debug("Date %s (day %d)" % (cdate.strftime("%Y-%m-%d"), wday))
        if wday in ssm.frequency_codes:
            ssmdates.append(cdate)
        cdate += datetime.timedelta(days=1)

    for flight_date in ssmdates:
        logger.debug("Delete flight date %s (day %s)" % (flight_date, flight_date))
        #DeleteSellingClasses(conn, ssm, flight_date)
        #DeleteFlightDate(conn, ssm.flight_number, flight_date)
