...
def ReadCodeShare(conn, flight_number, board_date_mdy):
    ...
    logger.info("Codeshares for flight %6s on %s:" % (flight_number, board_date_mdy))
    RcSql = \
...
def ReadFlightDeparture(conn, class_code, flight_number, flight_date):
    ...
    logger.info("Read data for flight %s date %s [flight_segment_dates]" % (flight_number, flight_date.strftime("%Y-%m-%d")))
    RcSql = \
        """SELECT departure_airport,arrival_airport, city_pair,
            departure_time,arrival_time,
            departure_terminal, arrival_terminal,
...
def ReadFlights(conn, flight_number, dt1, dt2, departure_airport, arrival_airport, code_share=False, class_code='Y', company_code='ZZ'):
    ...
    if dt1 is not None and dt2 is not None:
        ...
        logger.info("Read flights for date %s [flight_segment_dates]" % dt1.strftime("%Y-%m-%d"))
        FsegSql += \
...
def ReadFlight(conn, flight_number, dts, class_code='Y'):
    """Read flight."""
    logger.info("Read flight %s for date %s [flight_segment_dates]" % (flight_number, dts.strftime("%Y-%m-%d")))
    ...
    logger.info("Found %d flights for date %s" % (len(flights), dts.strftime("%Y-%m-%d")), 1)
    if len(flights):
        return flights[0]
    else:
        return None
...
def CheckFlight(conn, flight_number, dts, class_code='Y'):
    ...
    logger.info("Found %d flights for date %s" % (n, dts.strftime("%Y-%m-%d")))
    return n


def ReadFlightsDate(conn, dts, ndays, departure_airport, arrival_airport, code_share=False, class_code='Y', company_code='ZZ'):
    """Flights for date."""
    logger.info("Flights for date %s [flight_segment_dates]" % dts.strftime("%Y-%m-%d"))
    if code_share:
        logger.info("With codeshare")
    fdate=dts.strftime("%Y-%m-%d")
    FsegSql=\
    ...
    logger.info("Found %d flights for date %s" % (len(flights), dts.strftime("%Y-%m-%d")))
    return flights


def ReadFlightsDateLeg(conn, dts, ndays, departure_airport, arrival_airport, class_code='Y', company_code='ZZ'):
    """Flights for date."""
    logger.info("Flights for date %s [flight_segment_dates]" % dts.strftime("%Y-%m-%d"), 1)
    fdate=dts.strftime("%Y-%m-%d")
    FsegSql = \
