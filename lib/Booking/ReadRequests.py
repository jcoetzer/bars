...
def ReadRequests(conn, book_no, rqst_code, delim=" ", status_flag='A'):
    ...
    if book_no < 0:
        ...
        return 0, d_reqs
    logger.info("Read booking requests for booking %d" % (book_no))
    ssrSql = \
    ...
    n = 0
    for row in cur:
        d_reqs.append(str("%2s%1s%s%24s"
                          % (row['action_code'], row['actn_number'], delim,
                             row['request_text'])))
        n += 1
...
def ReadRequestsDaily(conn, recCount=0, start_date=None, end_date=None, dest_id=None):
    ...
    if recCount:
        FiSql += \
            " FIRST %d" % recCount
    FiSql += " book_no, rqst_code, action_code, actn_number, processing_flag," \
             " update_time, request_text," \
    ...
    # Run query
    cur.execute(FiSql)
    print("%8s %5s %4s %5s %8s %20s %s"
          % ("Booking", "SSR", "Stat", "User", "Dest", "Update", "Text"))
    for row in cur:
        print("%8d %5s %3s%1d %5s %8s %20s %s"
              % (row['book_no'], row['rqst_code'],
                 str(row['action_code'] or ''),
                 int(row['actn_number'] or 0),
                 row['update_user'], row['update_group'], row['update_time'],
                 row['request_text']))
    print
...
def ReadRequestCodes(conn, book_no, delim=" "):
    ...
    if book_no < 0:
        ...
        return 0, d_reqs
    logger.info("Read request codes for booking %d" % (bookno), 2)
    ssrSql = \
        "SELECT rqst_sequence_no,rqst_code,carrier_code,action_code,actn_number,request_text," \
        " processing_flag,all_passenger_flag,all_itinerary_flag" \
        " FROM book_requests WHERE book_no=%d" % book_no
    ...
    for row in cur:
        d_reqs.append(str("%4s%s%2s%d%s%-24s"
                      % (row['rqst_code'], delim, row['action_code'],
                         int(row['actn_number']), delim, row['request_text'])))
        n += 1

    logger.info("%d requests" % n)
    return n, d_reqs
