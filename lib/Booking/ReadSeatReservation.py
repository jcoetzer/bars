...
def CheckItenary(conn, flight_number, flight_date, depr_airport, book_no):
    """Itenary for booking."""
    logger.info("Itenary for booking %d flight %s on %s"
...
def CheckSeatReservationEtAsr(conn, flight, seat_row_code, bno):
    ...
    book_no = int(bno)
    logger.info("Check booking %d seat reservation for flight %s date %s seat %s" \
    ...
    logger.info("Seat row %d code %s" % (seat_row, seat_code))
    RcSql = \
    ...
    print("\tSeat reservations for booking %d for flight %s on %s seat %s :" \
        % (book_no, flight_number, flight_date.strftime("%Y-%m-%d"),
           seat_row_code)),
    for row in cur:
        print(" %s" % str(row['flight_seat_reservation_group_id'] or '')),
        n += 1
...
def CheckSeatReservation2(conn, flight_number, flight_date, depr_airport,
    ...
    logger.info("Check seat reservation for flight %s on %s seat %s" \
    ...
    print("\tNull reservations for flight leg %s on %s seat %s :" \
        % (flight_number, flight_date, seat_row_code)),
    for row in cur:
        print(" %s" % str(row['flight_seat_reservation_group_id'] or '')),
        n += 1
...
def ReadSeatReservationId(conn, flight_number, flight_date, status_flag,
    ...
    logger.info("Seat reservation ID for flight %s on %s :" \
...
def ReadSeatAttribute(conn, seat_definition_id):
    ...
    print("\t\tSeat attributes for seat definition ID %d : " % seat_definition_id),
    RcSql = \
    ...
    n = 0
    for row in cur:
        print("%s" % row['seat_attribute_rcd']),
        n += 1
    if n == 0:
        print("not found"),
    print('')
...
def ReadFlightSeatMap(conn, seat_map_id, recCount=1):
    ...
    print("\t\tFlight date legs for seat map ID %d :" % seat_map_id),
    RcSql = \
        "SELECT"
    if recCount:
        RcSql += \
    ...
    n = 0
    for row in cur:
        print(" %d" % row['flight_date_leg_id']),
        n += 1
    ...
    for row in cur:
        print("%s %s %s %s %s" \
            % (row['seat_number'], row['dcs_passenger_id'],
               row['dcs_itinerary_id'], row['boarding_control_number'])),
        n += 1
    if n == 0:
        print("not found"),
    print('')
